...
<template>
  <div class="flow-page">
    ...
    <section class="flow-section-card chart-card">
      <div class="flow-section-heading">
        ...
        <span class="chart-legend"
          ><i class="wake-dot"></i>{{ t('data.wake') }} <i class="sleep-dot"></i
          >{{ t('data.sleep') }}</span
        >
      </div>
      <div v-if="chartRows.length" class="chart-scroll">
        <svg
          ...
          role="img"
          :aria-label="t('data.wakeSleepCurve')"
        >
          <line
          ...
          <g v-for="point in chartPoints" :key="point.id">
            <circle
              ...
              :cy="point.wakeY"
              r="5"
              class="chart-point wake-point"
            />
            <circle
              v-if="point.sleepY !== undefined"
              :cx="point.x"
              :cy="point.sleepY"
              r="5"
              class="chart-point sleep-point"
            />
            <text
              :x="point.x"
              y="252"
    ...
    <section class="flow-data-grid">
      ...
      <article class="flow-section-card">
        ...
        <div
          ...
          :style="{ '--completion': `${completionPercent}%` }"
        >
    ...
    <section class="flow-section-card records-card">
      <div class="flow-section-heading">
        <div>
          <p class="flow-eyebrow">{{ t('data.records') }}</p>
      ...
      <div v-if="records.length" class="record-list">
        <article v-for="record in records" :key="record.id" class="record-row">
          <div class="record-main">
            ...
            <span
              >{{ t('data.wake') }} {{ record.wakeLabel }}
              <span v-if="record.sleepLabel"
                >· {{ t('data.sleep') }} {{ record.sleepLabel }}</span
              ></span
            >
          </div>
          <div class="record-meta">
            <span v-if="record.sleepMinutes">{{
              formatDuration(record.sleepMinutes)
            }}</span
            ><button
              class="flow-text-link"
              type="button"
    ...
    >
      <div class="flow-modal-panel">
        <div class="flow-modal-header">
          <div>
            <p class="flow-eyebrow">{{ t('data.edit') }}</p>
            <h2>{{ editingRecord.dateLabel }}</h2>
          </div>
          <button
            class="flow-icon-button"
            type="button"
            @click="editingRecord = undefined"
          >
            ×
          </button>
        </div>
        <form class="flow-form" @submit.prevent="saveRecord">
          <label
            ><span>{{ t('data.wake') }}</span
            ><input v-model="editWake" type="datetime-local" required
          /></label>
          <label
            ><span>{{ t('data.sleep') }}</span
            ><input v-model="editSleep" type="datetime-local"
          /></label>
...
const chartPoints = computed(() =>
  chartRows.value.map((session, index) => ({
    id: session.id,
    x: 48 + index * 72,
...
const wakeLine = computed(() =>
  chartPoints.value.map((point) => `${point.x},${point.wakeY}`).join(' '),
);
const sleepLine = computed(() =>
  chartPoints.value
    .filter((point) => point.sleepY !== undefined)
    .map((point) => `${point.x},${point.sleepY}`)
    .join(' '),
);

const orderedSessions = computed(() =>
  [...state.value.sessions].sort(
    (first, second) => Date.parse(first.wokeAt) - Date.parse(second.wokeAt),
  ),
);
const records = computed(() =>
  [...orderedSessions.value].reverse().map((session) => ({
    ...session,
    dateLabel: formatShortDate(new Date(session.wokeAt), locale.value),
    wakeLabel: formatClock(new Date(session.wokeAt), locale.value),
    sleepLabel: session.sleptAt
      ? formatClock(new Date(session.sleptAt), locale.value)
      : undefined,
    sleepMinutes: getSleepMinutes(
      session,
      orderedSessions.value[orderedSessions.value.indexOf(session) + 1],
    ),
  })),
);
...
const ratioRows = computed(() =>
  records.value
    .filter((record) => record.sleepMinutes && averageCycle.value)
    .slice(0, 8)
    .map((record) => {
      ...
      return {
        ...
        percent: Math.min(
          100,
          Math.round(
            (sleepMinutes / Math.max(1, averageCycle.value ?? 1)) * 100,
          ),
        ),
      };
    }),
...
const averageRatio = computed(() => {
  ...
  const total = ratioRows.value.reduce(
    (sum, row) => sum + Number(row.ratio.split(' ')[0]),
    0,
  );
...
const completedEvents = computed(
  () =>
    state.value.occurrences.filter(
      (occurrence) => occurrence.status === 'completed',
    ).length,
);
const allTrackedEvents = computed(
  () =>
    state.value.occurrences.filter(
...
const completionPercent = computed(() =>
  allTrackedEvents.value
    ? Math.round((completedEvents.value / allTrackedEvents.value) * 100)
    : 0,
);
...
const saveRecord = () => {
  ...
  updateSession(editingRecord.value.id, {
    ...
    sleptAt: editSleep.value
      ? new Date(editSleep.value).toISOString()
      : undefined,
  });

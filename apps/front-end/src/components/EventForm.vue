<!-- apps/front-end/src/components/EventForm.vue -->
<template>
  <section class="flow-modal" role="dialog" aria-modal="true" :aria-label="t('event.new')">
    <div class="flow-modal-panel">
      <div class="flow-modal-header">
        ...
        <button class="flow-icon-button" type="button" :aria-label="t('event.cancel')" @click="$emit('cancel')">×</button>
      </div>

      <form class="flow-form" @submit.prevent="submit">
        <label>
          <span>{{ t('event.title') }}</span>
          <input v-model.trim="form.title" required maxlength="120" autocomplete="off" />
        </label>
        <label>
        ...
        <fieldset>
          ...
          <div class="flow-choice-grid">
            <label class="flow-choice" :class="{ selected: form.schedule === 'wakeAfter' }">
              <input v-model="form.schedule" type="radio" value="wakeAfter" />
              <span>{{ t('event.afterWake') }}</span>
            </label>
            <label class="flow-choice" :class="{ selected: form.schedule === 'calendar' }">
        ...
        <label v-if="form.schedule === 'wakeAfter'">
          ...
          <input v-model.number="form.offsetHours" type="number" min="0.25" max="720" step="0.25" required />
        </label>
        <label v-else>
        ...
        <label>
          ...
          <select v-model="form.repeat">
            ...
            <option value="everyCycles">{{ t('event.repeat.everyCycles') }}</option>
          </select>
        </label>
        <label v-if="form.repeat === 'everyDays' || form.repeat === 'everyCycles'">
          <span>N</span>
          <input v-model.number="form.interval" type="number" min="1" max="365" step="1" required />
        </label>
        <label>
          <span>{{ t('event.reminder') }}</span>
          <div class="flow-input-with-suffix">
            <input v-model.number="form.reminderMinutes" type="number" min="0" max="1440" step="1" required />
            <span>{{ t('event.minutes') }}</span>
          </div>
        </label>

        <div class="flow-modal-actions">
          <button class="flow-button secondary" type="button" @click="$emit('cancel')">{{ t('event.cancel') }}</button>
          <button class="flow-button primary" type="submit">{{ t('event.save') }}</button>
        </div>
      </form>
    </div>
  </section>
...
const submit = () => {
  const firstDueAt = form.schedule === 'calendar' && form.firstDueAt
  ...
  emit('save', {
    ...
    offsetMinutes: form.schedule === 'wakeAfter' ? Math.round(form.offsetHours * 60) : undefined,
    firstDueAt,
    repeat: form.repeat,
